...
class spider():
    ...
    # 分析主页面 一共164个 7本田没爬完 10标志 109马自达没爬完
    def analysis_main_page(self):
        ...
        for i, (href, name) in enumerate(zip(hrefs, names)):
            if i in range(11, 100) and i not in []:
            # if i == 110:
        ...
        if not len(blocks) == 0:
            for block in blocks:
                ...
                for (href, subsub_name) in zip(hrefs, subsub_names):
                    ...
                    self.subsubsub_page(self.main_page + href, sub_path, subsub_name.strip())
        else:
            sub_path = path + '/' + name
            self.make_dir(sub_path)
            hrefs = selector.xpath(
                '//div[@class="brand_right"]/div[@class="choose_wrap clearfix"]/div[@class="car_list"]/div/a/@href')
            subsub_names = selector.xpath(
                '//div[@class="brand_right"]/div[@class="choose_wrap clearfix"]/div[@class="car_list"]/div/a/img/@title')
            for (href, subsub_name) in zip(hrefs, subsub_names):
                print("------->三级车型名字：{}, href: {}".format(subsub_name, href))
                self.subsubsub_page(self.main_page + href, sub_path, subsub_name.strip())
    ...
    def analysis_pages(self, url, folder_path, name):
        ...
        infoes = selector.xpath('//dl/dd/span[@class="info"]/text()')
        for href, ssssb_name, info in zip(hrefs, ssssub_names, infoes):
            # print(href, ssssb_name, info.strip())
            self.large_scale_iamge(self.main_page + href, path, ssssb_name, info.strip())

    # 获取大图
    def large_scale_iamge(self, url, folder_path, name, type):
        selector, path = self.get_selector_and_path(url, folder_path, name)
...
if __name__ == "__main__":
    sp = spider('http://newcar.xcar.com.cn', './xcar_download')
